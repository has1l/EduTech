...
`Shared/MathText.swift` — WKWebView + KaTeX (бандл, без CDN):
- Если текст содержит `\(` или `\[` → рендерит через KaTeX
- Иначе → обычный `Text` (без WKWebView overhead)
- KaTeX 0.16.9 бандлится в `Resources/KaTeX/` (katex.min.js, auto-render.min.js, katex.min.css, 20 woff2 шрифтов)
- `baseURL = Bundle.main.url(forResource: "katex.min", withExtension: "js")?.deletingLastPathComponent()` — загружает скрипты из файловой системы, не из сети
- В katex.min.css убран prefix `fonts/` из `@font-face` url() — все шрифты лежат рядом с CSS в корне бандла
- Высота WebView измеряется дважды: сразу и через 0.8 сек (локальные файлы грузятся быстро)
...
## 20. iOS — сессия задач (TaskSessionHost)

### Архитектура TaskSessionHost

`TaskSessionHost` — контейнер для свайп-сессии из нескольких заданий. Запускается через `Route.taskSession(allIds: [UUID], topicId: UUID?, origin: TaskOrigin)`.

**Навигация:** `TabView(selection: $currentPage).tabViewStyle(.page(indexDisplayMode: .never))` — единственный надёжный способ горизонтального свайпа рядом с вертикальным `ScrollView`. UIKit-оверлеи и `simultaneousGesture` не работают против `UIScrollView`.

**Кастомный топбар** (без navigationBar, используется `.toolbar(.hidden, for: .navigationBar)`):
```
[X в кружке]  [● ● ● ● ● +]  [Завершить]
[────── прогресс-бар ──────]
[divider]
[TabView с задачами]
```
...
- X слева: `router.pop()`
- Центр: `SessionDots` (скользящее окно) + иконка `+` для загрузки ещё 5 задач
- Справа: «Завершить» — серая до 5 решённых, жёлтая после

**Кнопка "+"`**: `GET /tasks/subtopic-session?topic_id=...&count=10`, фильтрует уже виденные ID, добавляет до 5 новых в `allIds`

**Завершение сессии (`finishSession()`):**
1. Все нерешённые задачи → `POST /booster` (reason: "ai" или "skipped")
2. Если ≥5 решено → `POST /streak/record`
3. `router.popToRoot()`

**Очистка кэшей:** `.onDisappear { TaskVMCache.shared.clear(); DialogueVMCache.shared.clear() }`

---

### SessionDots — скользящее окно кружочков

`SessionDots` (приватный struct в `TaskSessionHost.swift`) — показывает максимум 5 кружков, окно скользит вокруг `currentIndex`:
...
- Крайние кружочки в окне: opacity 0.35 (намёк что есть ещё)
- При сдвиге окна: `.transition(.scale(scale: 0.4).combined(with: .opacity))` — плавное появление/исчезновение
- `.animation(.smooth, value: currentIndex)` на HStack
- Ширина секции всегда фиксирована — кнопка «Завершить» не двигается

---

### TaskView — режим embedded

`TaskView` запускается в двух режимах:
- **Standalone** (NavigationStack push): свой toolbar с X и точками, `topProgressBar`, полная навигация
- **Embedded** (внутри `TaskSessionHost`): `isEmbedded = true` когда `onNext != nil`

В embedded режиме:
- `topProgressBar` скрыт (хост показывает свой прогресс)
- Toolbar скрыт (хост управляет навигацией)
- `goNext()` / `skipTask()` вызывают колбэки `onNext` / `onFinish` вместо router push
- В туторе: `compactTaskCard` рендерится **над** таб-свитчером (не внутри `tutorContent`)
- В `tutorContent`: `compactTaskCard` скрыт если embedded (уже показан выше)

---

### TaskVMCache и DialogueVMCache — сохранение контекста при свайпе

**Проблема:** TabView с PageTabViewStyle убивает страницы вне видимого окна и пересоздаёт их при возврате. `@State var vm` инициализируется заново → `load()` сбрасывает фазу.

**Решение:** два синглтона-кэша хранят живые VM по ID.

```swift
@MainActor final class TaskVMCache {
    static let shared = TaskVMCache()
    private var cache: [UUID: TaskVM] = [:]
    func getOrCreate(_ id: UUID, ...) -> TaskVM { ... }
    func clear() { cache.removeAll() }
}

@MainActor final class DialogueVMCache {
...
**`TaskVM.load()`:** `guard task == nil else { return }` — пропускает загрузку если задача уже есть, фаза (correct/dialogue/giveup) сохраняется.

**`DialogueVM.start()`:** `guard messages.isEmpty && !isStreaming else { return }` — не перезапускает стрим если уже идёт или есть сообщения.

**`DialogueView.onDisappear`:** убрано `vm.stop()` — стрим продолжается в фоне пока пользователь смотрит другое задание.

**Очистка:** при `.onDisappear` на `TaskSessionHost` — оба кэша сбрасываются.

---

### Подсказка свайпа (swipe hint)

`AppDefaults.didShowSwipeHint: Bool` — показывает подсказку «← свайпайте для смены задания →» один раз за установку приложения. Исчезает при свайпе или тапе. Хранится в `UserDefaults`.

---

## 21. iOS — путь (SessionPathView)

### Архитектура

`SessionPathView` — **корневой экран** домашнего таба (не пушится через роутер):
```swift
Tab("Сегодня", ...) {
...
Экран содержит два таба инлайн (не отдельные экраны):

```
[⚡ Путь | 🧠 Мой план]   ← таб-свитчер
[──────────────────────]
[   зигзаг-путь / план   ]   ← ScrollView меняется
```

- Таб-свитчер — точная копия стиля из `TaskView` (pill с фоном, spring-анимация)
- `StudyPlanView.swift` остаётся как отдельный navigation destination `.studyPlan`, но инлайн-план — `_EmbeddedPlanContent` (приватный struct в том же файле)
...
- Ноды расставляются с `VStack(spacing: 28)` внутри секции
- Каждая нода получает горизонтальный `.offset(x: zigzagOffsets[i % 8])`
- **Коннекторов нет** — ноды «плавают» как монетки Duolingo, без соединительных линий

---

### PathNodeView — 3D кнопка

Монетка создаётся двумя кругами в `ZStack`:

```swift
// Нижний слой (тень/ребро) — смещён вниз
Circle()
    .fill(darken(faceColor))   // на 35% темнее
    .frame(width: 72, height: 72)
    .offset(y: 5)

// Верхний слой (лицо кнопки)
Circle()
    .fill(faceColor)
    .frame(width: 72, height: 72)
...
|---|---|---|
| `current` | `appAccent` (жёлтый) | `bolt.fill` |
| `completed` | `appSuccess` (зелёный) | `checkmark` |
| `locked` | `Color(white: 0.84)` (серый) | `lock.fill` |

**Тень:** функция `darken(_:)` через UIColor HSB-компоненты:
```swift
...
Работает для любого цвета — не нужно хардкодить каждый оттенок тени.

**Пульсация** (только `current`): расширяющееся кольцо `appAccent.opacity(0.20)`, `.easeOut(duration: 1.6).repeatForever(autoreverses: false)`.

**Нет border overlay** — никаких `strokeBorder`, `StrokeStyle`, пунктирных рамок.

---

### _EmbeddedPlanContent

Приватный `struct _EmbeddedPlanContent: View` в `SessionPathView.swift`:
- `@Environment(Router.self)` + `@Environment(AppState.self)` — получает из окружения автоматически
- Три состояния: диагностика не пройдена → нет плана → карточки плана
- `groupCard` — приоритетный номер в чёрном кружке, статус-бейдж, AI-текст, полоска освоения через `GeometryReader`
- `generate()` → `POST /plan/generate`, `load()` → `GET /plan`

---

## 22. iOS — авторизация (LoginView)

### Архитектура

Экран входа показывается когда нет токена в Keychain. **Анонимная тихая регистрация убрана** — теперь пользователь всегда явно входит.

`AppState.bootstrap()`:
- Есть токен → `fetchMe()` → главное приложение
- Нет токена → `showLoginFlow = true` → показывается `LoginView`
- 401 при fetchMe → очищает токены → `showLoginFlow = true`

`RootView` дерево состояний:
```
isAuthenticating → BootstrapView (спиннер)
showLoginFlow    → LoginView
currentUser      → OnboardingFlow или MainTabsView
```

### LoginView

Файл: `Features/Auth/LoginView.swift`

- Два режима: **Войти** / **Регистрация** (pill-свитчер, аналог TabView в TaskView)
- Поля: Email + Пароль
- Кнопка **«Войти через Яндекс ID»** — показывается только если `!Config.yandexClientId.isEmpty`
- Ошибки API: 401 → "Неверный email или пароль", 409 → "Этот email уже зарегистрирован"
- `logout()` в AppState: сбрасывает токены + ставит `showLoginFlow = true`

### AppState — новые методы
...
### Яндекс ID через ASWebAuthenticationSession

`YandexAuthSession` — `@MainActor final class`, singleton, конформит `ASWebAuthenticationPresentationContextProviding`.

```swift
// Берёт ключевое окно для презентации
...
**Критично:** `session.presentationContextProvider = self` обязателен даже с новым `callback: .customScheme("edutech")` API (iOS 17.4+). Без него — `ASWebAuthenticationSessionError code 2` (presentationContextNotProvided).

**Completion handler** у нового `init(url:callback:completionHandler:)` — старый тип `(URL?, Error?)`, **не** `Result<URL, Error>`.

### Config.swift

```swift
static let yandexClientId  = ""                          // вставить из Яндекс OAuth консоли
static let yandexRedirectUri = "edutech://auth/yandex/callback"
```

`yandexClientId` — **публичный**, можно коммитить. `yandex_client_secret` — только на бэкенде (Railway).

### Настройка Яндекс OAuth консоли

1. Платформы → iOS-приложение: `2NXN9F6VAY.squad52.dev.EduTech`
2. Веб-сервисы → Redirect URI: добавить `edutech://auth/yandex/callback`
3. На Railway добавить: `YANDEX_REDIRECT_URI_MOBILE=edutech://auth/yandex/callback`

**Изменения в бэкенде (`auth.py`):** валидация redirect URI теперь принимает и веб, и мобильный URI:
```python
...
if body.redirect_uri and body.redirect_uri not in allowed:
    raise HTTPException(400, "Invalid Yandex redirect URI")
```

Изменения в Яндекс консоли применяются с задержкой до 10 минут.
